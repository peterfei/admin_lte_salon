...
(function() {
  ...
  this.flash = function(message, level, timeout) {
    var $flash, $last, flash_timeout, last_top;
    if (level == null) {
      level = 'info';
    }
    if (timeout == null) {
    ...
    last_top = $last.size() ? $last.position().top + $last.outerHeight() - $('body').scrollTop() : 0;
    $flash = $("<div class=\"alert alert-" + level + " fade in flash-message\">\n  <button type=\"button\" class=\"close\">&times;</button>\n  " + message + "\n</div>").css('top', last_top + 10).appendTo('.modal-header').fadeIn();
    flash_timeout = timeout || TIMEOUT_MAPPINGS[level] || 2000;
    if (timeout !== false) {
      return $flash.delay(flash_timeout).fadeOut(function() {
        return $(this).remove();
      });
    }
  };
